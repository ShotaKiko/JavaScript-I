...
let oldCars =[];
let ii = carYears[0];
for(let ii = 0; ii < carYears.length; ii++){
    if(carYears[ii] < 2000){
        oldCars.push(carYears[ii])
    }
}
...
let BMWAndAudi =[];
let iii = inventory[0]
for(let iii = 0; iii < inventory.length; iii++){
    if(inventory[iii].car_make === 'Audi' || inventory[iii].car_make === 'BMW'){
        BMWAndAudi.push(inventory[iii])
    }
}
